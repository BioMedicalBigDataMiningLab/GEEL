...
def construct_net(train_LMI, positive_num):  #
    miRNA_seq = np.loadtxt('D:/mi_seq_L.csv', delimiter=',', dtype=float)
    lncRNA_seq = np.loadtxt('D:/lnc_seq_L.csv', delimiter=',', dtype=float)
...
def net2edgelist(lncRNA_miRNA_matrix_net):  #
    ...
    np.savetxt('D:/L_lncRNA_miRNA_edgelist_L.txt', none_zero_dgelist, fmt="%d", delimiter=' ')


###############################################################################################
def get_dw_embedding_matrix(lncRNA_miRNA_matrix_net, graph1):
    model = node2vec.Node2vec(graph=graph1, path_length=80, num_paths=30, dim=30, dw=True)  # deepwalk
    vec = model.vectors
...
def get_GraRep_embedding_matrix(lncRNA_miRNA_matrix_net, graph1):
    model = grarep.GraRep(graph=graph1, Kstep=1, dim=120)  # best parameter
    vec = model.vectors
    matrix = np.zeros((len(lncRNA_miRNA_matrix_net), len(list(vec.values())[0])))
    for key, value in vec.items():
        matrix[int(key), :] = value
...
def get_GAE_embedding_matrix(lncRNA_miRNA_matrix_net, hidden_unit=512):
    cmd_path1 = 'start cmd /k bionev --input D:/L_lncRNA_miRNA_edgelist_L.txt --output D:/GAE_emb_L.txt  --method GAE  --dimensions 120  --gae_model_selection gcn_vae  --epochs 500  --hidden %s'%(str(hidden_unit))
    os.system(cmd_path1)
    print('wait 35 second for  D:/GAE_emb_L.txt')
    time.sleep(35)
    vec = get_embddeing_from_txt('D:/GAE_emb_L.txt')
...
def get_individual_emb(train_lncRNA_miRNA_matrix, positive_num):
    ...
    graph1 = graph.Graph()
    graph1.read_edgelist("D:/L_lncRNA_miRNA_edgelist_L.txt")
    ...
    GAE_lncRNA_miRNA_emb = get_GAE_embedding_matrix(np.mat(lncRNA_miRNA_matrix_net), hidden_unit=512)   #  using a different function
...
def holdout_by_link(train_lncRNA_miRNA_matrix, ratio, seed, zero_row_index, zero_col_index):
    ...
    # 注意：这里的0，是用的最原始的0，没有将测试集中的1（1/5的1）置0
...
def ensemble_method(lncRNA_miRNA_matrix, train_row_shuffle, train_col_shuffle, testPosition, seed,
    ...
    [dw_test_feature, dw_test_label] = get_test_data(lncRNA_miRNA_matrix, lncRNA_emb_dw, miRNA_emb_dw, testPosition)  #  注意 label
...
def ensemble_scoring(real_matrix, multiple_matrix, testPosition, LR_cf1):  # 计算5种集成方法的scores
    real_labels = []
    ...
    for i in range(0, len(testPosition)):
        ...
        for indivadual_matrix in multiple_matrix:
            vector.append(indivadual_matrix[i])  # 顺序是对应的，已经check，因为np.where顺序固定
...
def get_LR_clf(real_matrix, multiple_matrix, testPosition):
    ...
    clf1 = linear_model.LogisticRegression(C=1.0, penalty='l2', tol=1e-6)  # 默认参数 基本没有调参
...
def cross_validation_experiment(lncRNA_miRNA_matrix, seed, k_folds):
    ...
    for k in range(k_folds):
        ...
        # testing set： testPosition2 训练集的1/5的1 和 原始的全部的0
        testPosition2 = []
        ...
        ###################    先训练一次，得到集成训练好的 集成model     ######################################
        # testPosition1 含有正负样本,用于训练 集成model
    ...
    df.to_csv('D:/5_all_emb_L/'+'seed_' + str(seed) + '.csv')
    return metric
...
if __name__ == "__main__":
    k_folds = 5
    result_csv = []
    l_m_matrix = np.loadtxt('D:/LMI_L.csv', delimiter=',', dtype=int)
    ...
    df = pd.DataFrame(np.array(result_csv),
                      columns=['AUPR', 'AUC', 'F1', 'ACC', 'REC', 'SPEC', 'PRE'])
    df.to_csv('D:/5_all_emb_L/20_seed.csv')
